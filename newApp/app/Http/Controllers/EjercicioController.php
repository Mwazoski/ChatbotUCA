...
class EjercicioController extends Controller
{
    ...
    public function ajaxFormularioQuery(Request $request)
    {
        ...
        if (
            stripos($SecPass[0], 'insert') !== false || stripos($SecPass[0], 'delete') !== false || stripos($SecPass[0], 'update') !== false
        ...
        try {
            ...
            if ($users1 == $sol1 && $users2 == $sol2) { //Consulta correcta
                ...
                elseif (array_key_exists('show', $arrayQueryUser[0])) {
                    ...
                    $mensaje = false;
                    array_push($respuestaQuery, array("query" => $users1, "conversacionBot" => "comprobacion_query laravel"));
                    if (selectsEnDescribeOShow($arraySolucion[0], $mejoraConsulta)) {
                        $mensaje = true;
                    } elseif (comprobacionesUnion($stringUsuario, $solucionQuery, $arrayQueryUser, $arraySolucion, $mejoraConsulta)) {
                        $mensaje = true;
                    } else {
                        ...
                        if (comprobacionesSelect($selectUser, $selectSol, $camposUser, $camposSol, $mejoraConsulta)) {
                            $mensaje = true;
                        } elseif (comprobacionesFrom($fromUser, $fromSol, $mejoraConsulta)) {
                            $mensaje = true;
                            ...
                            if (!array_key_exists('order by', $arrayQueryUser[0])) {
                                ...
                                $mensaje = true;
                            } elseif (comprobacionesOrderBy($arrayQueryUser[0]['order by'], $arraySolucion[0]['order by'], $camposUser, $camposSol, $mejoraConsulta)) {
                                $mensaje = true;
                            }
                            ...
                            if (!array_key_exists('group by', $arrayQueryUser[0])) {
                                ...
                                $mensaje = true;
                            } elseif (comprobacionesGroupBy($arrayQueryUser[0]['group by'], $arraySolucion[0]['group by'], $camposUser, $camposSol, $mejoraConsulta)) {
                                $mensaje = true;
                            }
                        }
                        if (!$mensaje && comprobacionesWhereHaving($users1, $users2, $sol1, $sol2, $mejoraConsulta)) {
                            $mensaje = true;
                        } elseif (!$mensaje) {
...
function comprobacionesSelect($selectUser, $selectSol, $camposUser, $camposSol, &$mejoraConsulta)
{
    ...
    if ((stripos($selectSol, "distinct") !== false) !== (stripos($selectUser, "distinct") !== false)) {
        ...
        $v = true;
    } elseif (compruebaFunciones($camposSol, $camposUser, $mejoraConsulta)) {
        $v = true;
...
function comprobacionesOrderBy($arrayQueryUser, $arraySolucion, $camposUser, $camposSol, &$mejoraConsulta)
{
    ...
    $arr1 = preg_split('/\s+,\s+/', preg_replace_callback('/\sasc\s?$/', function ($coincidencias) {
        return " ";
    }, $arr1));
    $arr2 = preg_split('/\s+,\s+/', preg_replace_callback('/\sasc\s?$/', function ($coincidencias) {
        return " ";
    }, $arr2));
...
function comprobacionesFrom($arrayQueryUser, $arraySolucion, &$mejoraConsulta)
{
    ...
    if (!empty(array_diff($a1, $a2)) || !empty(array_diff($a2, $a1))) {
        ...
        if (
            !$v && (stripos($arrayQueryUser, 'left') !== false || stripos($arrayQueryUser, 'right') !== false) &&
...
function getCampos($str)
{
    ...
    $res = str_replace(' ', '', $str);
    $res = preg_replace_callback('/\'.+\',/', function ($coincidencias) {
        return ' ';
    }, $res);
    $res = preg_replace_callback('/\'.+\'/', function ($coincidencias) {
        return '';
    ...
    $res = preg_replace_callback('/,(?![^\(]*\))/', function ($coincidencias) {
        return ' ';
    }, $res); //En caso de que no haya renombres
